...
% If the game is not clear or lost, continue
singleGame(Board, Difficult, Name, StartTime) :-
    sizeDifficulty(W, L, Difficult),
...
% Check if the multiplayer game is clear
multiGame(Board, _, _) :-
    winGame(Board),
    write("ALL Clear!!!"), nl, 
    printBoard(Board, show),           % show means show mines, hide will hide mines
...
% Check if the multiplayer game is lost
...
% In main function, if user enters 4, quit the game.
returnIf4(X) :-
...
% Click a grid for single player
...
% Set/ Cancel flag for a single player
...
% expandclick(lst, oldBoard, newBoard), Return a new game board after clicking all lst
