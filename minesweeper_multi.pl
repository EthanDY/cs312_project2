...
write("Single player: 0.\nDual player:   1."), nl,
modeInput(Mode),
get_time(StartTime),
singleGame(NBoard, Difficult, Name, StartTime, Mode),
multiGame(NBoard, Difficult, Name, StartTime, Mode), !.

singleGame(_, _, _, _, 1).

singleGame(Board, Difficult, Name, StartTime, 0) :-
winGame(Board),
write("You Win!!!"), nl,
...
singleGame(Board, Difficult, _, _, 0) :-
...
singleGame(NNBoard, Difficult, Name, StartTime, 0), !.

multiGame(_, _, _, _, 0).

multiGame(Board, Difficult, Name, StartTime, 1) :-
winGame(Board),
write("ALL Clear!!!"), nl,
printBoard(Board, show),           % show means show mines, hide will hide mines
writeRecords(Name, Difficult, StartTime),
...
multiGame(Board, Difficult, _, _, 1) :-
printLoseGame(Board, Difficult),
printBoard(Board, show),
pScore(Board),
write("Do you wish to continue?\nYes:     0.  \nNo:      1."), nl,
...
sizeDifficulty(W, L, Difficult),
printBoard(Board, show),
pScore(Board),          % show means show mines, hide will hide mines
NW is W - 1, NL is L - 1,
%write("B Set a flag?\nYes:     1.  \nNo:      0."), nl,
%getValidInput(Input, 0, 1),
dealWithInputA(0, NW, NL, Board, NBoard),
printBoard(NBoard, show),
pScore(NBoard),
%write("B Set a flag?\nYes:     1.  \nNo:      0."), nl,
%getValidInput(Input2, 0, 1),
dealWithInputB(0, NW, NL, NBoard, NNBoard),
multiGame(NNBoard, Difficult, Name, StartTime, 1), !.
...
write("Flag Please input a Row: "), nl,
getValidInput(X1, 0, NW),
write("Flag Please input a Column: "), nl,
getValidInput(Y1, 0, NL),
flag(X1, Y1, NBoard, NNBoard), !.

dealWithInputA(0, NW, NL, NBoard, NNBoard) :-
write("A Please input a Row: "), nl,
getValidInput(X, 0, NW),
write("A Please input a Column: "), nl,
getValidInput(Y, 0, NL),
expand([],[(X,Y)],NBoard, B),
expandClickA(B, NBoard, NNBoard),!.

dealWithInputB(0, NW, NL, NBoard, NNBoard) :-
write("B Please input a Row: "), nl,
getValidInput(X, 0, NW),
write("B Please input a Column: "), nl,
getValidInput(Y, 0, NL),
expand([],[(X,Y)],NBoard, B),
...
string_concat(NumSS, "]", NumSSS),
string_concat(NumSSS, NS, S), !.
generateRow([grid(_, _, _, true, true, _, _, _, 0)|T], S, O) :-
generateRow(T, NS, O),
string_concat("[N]", NS, S), !.
...
clickRowA(_, [], []).
clickRowA(Col, [grid((Row,Col), A, B, _, D, E, F, G, H)|T], GridRow) :-
...
clickRowB(_, [], []).
clickRowB(Col, [grid((Row,Col), A, B, C, _, E, F, G, H)|T], GridRow) :-
